...
def exact_deduplicate(documents: list[dict]) -> tuple[list[dict], list[dict]]:
    seen_texts = {}  # Menggunakan Dictionary untuk melacak teks -> dokumen asli
    unique_documents = []
    removed_documents = []

    for doc in documents:
        # Normalisasi teks: huruf kecil semua dan penyeragaman spasi
        normalized_text = " ".join(doc["text"].lower().split())
        
        # Pencocokan langsung (Exact String Matching)
        if normalized_text not in seen_texts:
            seen_texts[normalized_text] = doc
            unique_documents.append(doc)
        else:
            # Jika teks sudah ada di memori, tandai sebagai duplikat
            original = seen_texts[normalized_text]
...
def save_dedup_results(unique_documents: list[dict], removed_documents: list[dict]):
    ...
    report = {
        ...
        "metode": "Exact string matching (normalized text)",
        "detail_dibuang": [
            {
                "dokumen_dibuang": {
