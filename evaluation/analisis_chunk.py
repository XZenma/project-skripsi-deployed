...
setiap ambang batas (20, 30, 50, 75, 100 karakter) serta detail chunk terpanjang.
...
def analisis_teknik(teknik: str) -> dict:
    ...
    statistik = {
        ...
        "rata_rata": round(sum(lengths) / n, 2) if n else None,
        "median": lengths_sorted[n // 2] if n else None,
    }

    # --- ekstraksi 1 chunk terpanjang ---
    chunk_terpanjang = {}
    if n > 0:
        longest_entry = max(docstore, key=lambda entry: len(entry.get("text", "")))
        teks_max = longest_entry.get("text", "")
        chunk_terpanjang = {
            "filename": longest_entry.get("filename"),
            "matkul": longest_entry.get("matkul"),
            "index_chunk": longest_entry.get("index_chunk"),
            "panjang_karakter": len(teks_max),
            "text": teks_max
        }
    ...
    # --- deteksi near-duplicate whitespace-only ---
    hash_normed = {}
    near_duplicates = []
    for entry in docstore:
        teks = entry.get("text", "")
    ...
    return {
        "teknik": teknik,
        "statistik": statistik,
        "chunk_terpanjang": chunk_terpanjang,
...
def main():
    ...
    for teknik, hasil in hasil_akhir.items():
        ...
        print(f"\n{teknik.upper()} (total {stat['total_chunk']} chunk, min={stat['min']}, max={stat['max']}, "
              f"rata_rata={stat['rata_rata']}, median={stat['median']})")
        
        # Tampilkan info chunk terpanjang di terminal
        terpanjang = hasil["chunk_terpanjang"]
        if terpanjang:
            print(f"  [!] Chunk Terpanjang: {terpanjang['filename']} (Index: {terpanjang['index_chunk']})")
            print(f"      Matkul: {terpanjang['matkul']} | Kapasitas: {terpanjang['panjang_karakter']} karakter")
            # Menampilkan potongan teks 120 karakter pertama agar terminal tetap rapi
            preview_teks = normalisasi_ringan(terpanjang['text'])[:120]
            print(f"      Preview Teks: \"{preview_teks}...\"")
            
        print("  Distribusi ukuran kecil:")
        for ambang in AMBANG_UJI:
